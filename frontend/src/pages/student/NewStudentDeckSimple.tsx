...
export default function NewStudentDeckSimple() {
  ...
  const loadDecks = async () => {
    try {
      ...
      if (response.success && response.decks) {
        // Filtrar apenas decks do usuário ou públicos
        const userDecks = response.decks.filter(deck => 
          deck.is_public || deck.user_id === user?.id
        );
        setAvailableDecks(userDecks);
      }
    } catch (error) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-300 font-police-subtitle">
                  Configure seu arsenal de estudos
                </p>
              </div>
            </div>
          </div>
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Organize seus estudos criando decks dentro de decks
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              <AnkiImportExport
                flashcards={[]}
                deckName={deckName || "Novo Arsenal"}
                onImport={(imported) => {
                  // Adicionar flashcards importados à lista disponível
                  const newFlashcards = imported.map((card, idx) => ({
                    ...card,
                    id: `imported_${Date.now()}_${idx}`,
                    category: selectedCategory || card.category || 'Importado'
                  }));
                  setAvailableFlashcards(prev => [...prev, ...newFlashcards]);
                  // Selecionar automaticamente os novos flashcards
                  const newIds = newFlashcards.map(f => f.id);
                  setSelectedFlashcards(prev => [...prev, ...newIds]);
                  toast.success(`${imported.length} flashcards importados e selecionados!`);
