...
export default function CoursesPage() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (selectedCategories.length > 0) {
      filtered = filtered.filter(course => {
        // Verificar se a categoria do curso corresponde a alguma categoria selecionada
        return selectedCategories.some(catId => {
          const category = findCategoryById(catId);
          if (!category) return false;
          
          // Verificar se a categoria do curso corresponde ao nome da categoria selecionada
          // ou suas subcategorias
          const categoryName = category.name.toUpperCase();
          const allChildrenIds = findAllChildren(catId);
          const allChildrenNames = allChildrenIds.map(id => {
            const cat = findCategoryById(id);
            return cat?.name.toUpperCase();
          }).filter(Boolean);
          
          return course.category === categoryName || 
                 allChildrenNames.includes(course.category) ||
                 course.subcategory === categoryName ||
                 allChildrenNames.includes(course.subcategory || '');
        });
      });
    }
    ...
    return filtered;
  }, [courses, searchTerm, selectedCategories, selectedLevel, sortBy, showOnlyEnrolled]);
              ...
              <span className="flex items-center gap-2">
                ...
                {selectedCategories.length === 0
                  ...
                  : selectedCategories.length === 1
                    ? findCategoryById(selectedCategories[0])?.name.toUpperCase() || 'OPERAÇÃO'
                    : `${selectedCategories.length} OPERAÇÕES`
                }
              </span>
              <ChevronDown className="w-4 h-4" />
            </button>
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar áreas operacionais..."
              ...
              <div className="max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                {filteredCategories.length === 0 ? (
                  <p className="text-gray-500 dark:text-gray-400 text-center py-8 font-police-body">
                    Nenhuma área operacional encontrada
                  </p>
                ) : (
